...
# Build desde el directorio padre: docker build -f boogiepop-stock-market/Dockerfile -t stock-market .
...
COPY boogiepop-stock-market/package.json ./
...
WORKDIR /workspace/app
COPY boogiepop-stock-market/ .

ARG VITE_REMOTE_BASE=/
ENV VITE_REMOTE_BASE=${VITE_REMOTE_BASE}

ARG NGINX_BASE_PATH=stock-market
...
COPY boogiepop-stock-market/nginx.conf /etc/nginx/conf.d/default.conf
